	...
	switch (m) {
		...
		case WM_CREATE:
			...
			*/
			...
			if(global_fSecondsRecord>0)
			{
				global_timer=timeSetEvent(global_fSecondsRecord*1000,25,(LPTIMECALLBACK)&StopRecording,0,TIME_ONESHOT);
			}
...
int PASCAL WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpCmdLine, int nCmdShow)
{
	...
	if(argCount>2)
	{
		global_fSecondsRecord = atof((LPCSTR)(szArgList[2]));
	}
	if(argCount>3)
	{
	...
	it = global_devicemap.find(global_devicename);
